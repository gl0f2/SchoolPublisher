...
def pdfs_zusammenfuehren(pdf_dateien: list[Path], ziel: Path) -> Path | None:
    """Führt die Klassen-PDFs plattformunabhängig mit pypdf zusammen."""
...
def main() -> None:
    ...
    try:
        ...
        # Optional gezielt Klassen über die Kommandozeile auswählen.
        alle_klassen = projekt.schule.klassen()
        if len(sys.argv) > 1:
            gewuenschte_klassen = [arg.strip() for arg in sys.argv[1:] if arg.strip()]
            klassen_lookup = {name.casefold(): name for name in alle_klassen}
            unbekannte_klassen = [
                name for name in gewuenschte_klassen
                if name.casefold() not in klassen_lookup
            ]
            if unbekannte_klassen:
                print("\nUnbekannte Klasse(n): " + ", ".join(unbekannte_klassen))
                print("Verfügbare Klassen: " + ", ".join(alle_klassen))
                return

            klassen_zum_rendern = [
                klassen_lookup[name.casefold()]
                for name in gewuenschte_klassen
            ]
            print("\nGezielte Ausgabe für: " + ", ".join(klassen_zum_rendern))
        else:
            klassen_zum_rendern = alle_klassen

        print("\nDokumente werden erstellt:\n")
        erzeugte_pdfs: list[Path] = []
        for klassenname in klassen_zum_rendern:
        ...
        # Gesamt-PDF nur bei vollständiger Ausgabe aller Klassen neu erzeugen.
        if len(sys.argv) == 1:
            ...
            if gesamt_pdf:
                ...
                print("Hinweis: Gesamt-PDF konnte nicht erzeugt werden.")
        else:
            print("→ Gesamt-PDF bei gezielter Klassenauswahl nicht neu erzeugt.")

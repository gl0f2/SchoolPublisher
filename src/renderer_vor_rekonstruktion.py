...
class ElternabendHtmlRenderer:
    ...
    def rendern(
        ...
        for fachname in sortierte_fachnamen:
            ...
            # 4x3-Layout: keine erzwungenen Doppelkarten.
            # Breite Karten entstehen nur automatisch bei 3 oder mehr Lehrkräften.
        ...
        # 4 x 3 Raster = 12 Plätze. Bei Überlauf wird auf Seite 1
        # genau ein Platz unten rechts für die Fortsetzung reserviert.
        # Breite Karten zählen als zwei Rasterplätze.
        gesamtplaetze = sum(breite for _, breite in karten_mit_breite)
        if gesamtplaetze > 12:
            erste_karten: list[str] = []
            restliche_karten: list[tuple[str, int]] = []
            belegte_plaetze = 0
            im_rest = False
            for karten_html, breite in karten_mit_breite:
                if not im_rest and belegte_plaetze + breite <= 11:
                    erste_karten.append(karten_html)
                    belegte_plaetze += breite
                else:
                    im_rest = True
                    restliche_karten.append((karten_html, breite))
            ...
            fortsetzungskarte = f"""
<article class="card continuation-card">
...
"""
            erste_karten.append(fortsetzungskarte)
            kartenseiten = [erste_karten]
            kartenseiten.extend(
                self._karten_auf_seiten(restliche_karten, maximale_plaetze=12)
            )
        else:

...
class ElternabendHtmlRenderer:
    ...
    def rendern(
        ...
        for fachname in sortierte_fachnamen:
            ...
            klasse_key = klassenname.strip().casefold()
            fach_key = fachname.strip().casefold()
            # Im 4×3-Raster werden auch Karten mit mehreren Lehrkräften normal dargestellt.
            ist_doppelkarte = False
            kompakter_kopf = (
                (klasse_key in {"5c", "6a", "6b"} and fach_key == "musik")
                or (klasse_key == "7d" and fach_key == "religion (ev.)")
            )
            ...
            karten_mit_breite.append(
                (
                    self._fachkarte_erstellen(
                        ...
                        fachname=fachname,
                        eintraege=eintraege,
                        breite_erzwungen=ist_doppelkarte,
                        kompakter_kopf=kompakter_kopf,
                    ),
                    2 if anzahl_lehrkraefte >= 3 or ist_doppelkarte else 1,
                )
            )
        ...
        # Bis 12 Rasterplätze: normale 4×3-Verteilung.
        # Bei mehr Inhalt reservieren wir unten rechts exakt einen Platz
        # für die Fortsetzungskarte. Breite Fachkarten zählen als 2 Plätze.
        gesamtplaetze = sum(breite for _, breite in karten_mit_breite)

        if gesamtplaetze > 12:
            erste_seite_mit_breite: list[tuple[str, int]] = []
            belegte_plaetze = 0
            trennindex = 0

            # Maximal 11 Rasterplätze mit echten Fachkarten belegen.
            # Platz 12 bleibt fest für die Fortsetzungskarte unten rechts.
            for index, (karten_html, breite) in enumerate(karten_mit_breite):
                breite = max(1, breite)
                if belegte_plaetze + breite > 11:
                    break
                erste_seite_mit_breite.append((karten_html, breite))
                belegte_plaetze += breite
                trennindex = index + 1

            restliche_karten = karten_mit_breite[trennindex:]
            anzahl_weitere_faecher = len(restliche_karten)
            bezeichnung_weitere = (
                "1 weiteres Fach"
                if anzahl_weitere_faecher == 1
                else f"{anzahl_weitere_faecher} weitere Fächer"
            )

            fortsetzungskarte = f"""
            <article class="subject-card continuation-card">
              <div class="continuation-inner">
                <div class="continuation-arrow">→</div>
                <div class="continuation-title">{bezeichnung_weitere}</div>
                <div class="continuation-text">Die Übersicht wird auf der Rückseite fortgesetzt.</div>
              </div>
            </article>
            """

            erste_seite = [
                karten_html for karten_html, _ in erste_seite_mit_breite
            ]
            erste_seite.append(fortsetzungskarte)

            # Die Rückseite darf wieder die vollen 12 Rasterplätze nutzen.
            weitere_seiten = self._karten_auf_seiten(
                restliche_karten,
                maximale_plaetze=12,
            )
            kartenseiten = [erste_seite, *weitere_seiten]
        else:
...
"""
    ...
    def _fachkarte_erstellen(
        ...
        ist_hauptklassenlehrer_fach = any(
            lehrkraft.kuerzel.strip().casefold() == hauptklassenlehrer
            for lehrkraft in lehrkraefte
        ) if hauptklassenlehrer else False
        ...
        if kompakter_kopf:
            namen_kopf = " · ".join(
                lehrername(lehrkraft) for lehrkraft in lehrkraefte
            )
            return f"""
            <article class="card{artikelklasse}" style="--accent:{accent}">
                <div class="card-head" style="display:flex; align-items:center; gap:1.6mm;">
                    <div class="subject-icon">{escape(icon)}</div>
                    <h3 class="subject" style="margin:0; white-space:nowrap;">{escape(fachanzeige)}</h3>
                    <p class="teacher" style="margin:0 0 0 auto; text-align:right; line-height:1.05;">{namen_kopf}</p>
                </div>
                <div class="card-main{mehrfach}{hauptklasse}" style="padding-top:1mm;">
                    <div class="photos{multi}{anzahlklasse}">{bilder_html}</div>
                    <div class="details">
                        {bewertung_html}
                    </div>

...
class ElternabendHtmlRenderer:
    ...
    def rendern(
        ...
        for fachname in sortierte_fachnamen:
            ...
            ist_doppelkarte_klasse_10 = (
                klassenname.strip().casefold().startswith("10")
                and fachname.strip().casefold() in {"imp", "sport (profil)"}
            )
            ...
            karten_mit_breite.append(
                (
                    self._fachkarte_erstellen(
                        ...
                        fachname=fachname,
                        eintraege=eintraege,
                        breite_erzwungen=ist_doppelkarte_klasse_10,
                    ),
                    2 if anzahl_lehrkraefte >= 3 or ist_doppelkarte_klasse_10 else 1,
                )
            )
